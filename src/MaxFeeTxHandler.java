...
public class MaxFeeTxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
        UTXOPool distinctUtxs = new UTXOPool();
        double previousTranSum = 0;
        double currentTranSum = 0;
        for (int i = 0; i < tx.numInputs(); i++) {
            ...
            if (!Crypto.verifySignature(output.address, tx.getRawDataToSign(i), in.signature))
                return false;
            if (distinctUtxs.contains(utxo)) return false;
            distinctUtxs.addUTXO(utxo, output);
            previousTranSum += output.value;
        }
        for (Transaction.Output out : tx.getOutputs()) {
            if (out.value < 0) return false;
            currentTranSum += out.value;
        }
        return previousTranSum >= currentTranSum;
    }

    private double calcTranFees(Transaction tx) {
        double sumInputs = 0;
        double sumOutputs = 0;
    ...
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        ...
        Set<Transaction> txsSortedByFees = new TreeSet<>((tx1, tx2) -> {
            double tx1Fees = calcTranFees(tx1);
            double tx2Fees = calcTranFees(tx2);
