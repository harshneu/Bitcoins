...
public class TxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
        UTXOPool distinctutqos = new UTXOPool();
        double previousTextSum = 0;
        double currentTextSum = 0;
        for (int i = 0; i < tx.numInputs(); i++) {
            ...
            if (!Crypto.verifySignature(output.address, tx.getRawDataToSign(i), in.signature))
                return false;
            if (distinctutqos.contains(utxo)) return false;
            distinctutqos.addUTXO(utxo, output);
            previousTextSum += output.value;
        }
        for (Transaction.Output out : tx.getOutputs()) {
            if (out.value < 0) return false;
            currentTextSum += out.value;
        }
        return previousTextSum >= currentTextSum;
    }
